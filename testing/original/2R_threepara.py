...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    diffusion_net_u1 = instantiate_arch(
        input_keys=[Key("x"), Key("Q"), Key("Sigma_a1"), Key("Sigma_a2")],
        output_keys=[Key("u1")],
    ...
    def lhs_interior_points(x_bounds, Q_bounds, Sa1_bounds, Sa2_bounds, n_points, seed=None):
    ...
    interior1 = PointwiseConstraint.from_numpy(
        nodes=nodes,
        invar={"x": x1, "Q": Q1, "Sigma_a1": Sa1_1, "Sigma_a2": Sa2_1},
    ...
    interior2 = PointwiseConstraint.from_numpy(
        nodes=nodes,
        invar={"x": x2, "Q": Q2, "Sigma_a1": Sa1_2, "Sigma_a2": Sa2_2},
...
######################################################################################################
    ...
    all_x_u1, all_u1_vals, all_Q_u1, all_Sigma_a1_u1, all_Sigma_a2_u1 = [], [], [], [], []
    all_x_u2, all_u2_vals, all_Q_u2, all_Sigma_a1_u2, all_Sigma_a2_u2 = [], [], [], [], []
    ...
    for Q_val in [0.2, 0.5, 0.7]: 
        for Sigma_a1_val in [0.02, 0.05, 0.08]:
            for Sigma_a2_val in [0.2, 0.3, 0.4]:
              u1 = analytical_solution_1(
                          X1.flatten(), D1, D2, a_ext, Sigma_a1_val, Sigma_a2_val, Q_val, a1
                      )
              validator = PointwiseValidator(
                  nodes=nodes, invar={
                      "x": X1.reshape(-1,1), "Q": np.full((len(X1),1), Q_val), "Sigma_a1": np.full((len(X1),1), Sigma_a1_val), "Sigma_a2": np.full((len(X1),1), Sigma_a2_val)
                  },
              ...
              u1_pts = analytical_solution_1(x_pts, D1, D2, a_ext, Sigma_a1_val, Sigma_a2_val, Q_val, a1)
              for x_pt, u_pt in zip(x_pts, u1_pts):
                  all_x_u1.append([x_pt])
                  all_Q_u1.append([Q_val])
                  all_Sigma_a1_u1.append([Sigma_a1_val])
                  all_Sigma_a2_u1.append([Sigma_a2_val])
    ...
    for Q_val in [0.2, 0.5, 0.7]:
        for Sigma_a1_val in [0.02, 0.05, 0.08]:
            for Sigma_a2_val in [0.1, 0.2, 0.3]:
              u2 = analytical_solution_2(
                          X2.flatten(), D1, D2, a_ext, Sigma_a1_val, Sigma_a2_val, Q_val, a1
                      )
      
              validator = PointwiseValidator(
                  nodes=nodes, invar={
                      "x": X2.reshape(-1,1), "Q": np.full((len(X2),1), Q_val), "Sigma_a1": np.full((len(X2),1), Sigma_a1_val), "Sigma_a2": np.full((len(X2),1), Sigma_a2_val)
                  },
              ...
              u2_pts = analytical_solution_2(x_pts, D1, D2, a_ext, Sigma_a1_val, Sigma_a2_val, Q_val, a1)
              for x_pt, u_pt in zip(x_pts, u2_pts):
                  all_x_u2.append([x_pt])
                  all_Q_u2.append([Q_val])
                  all_Sigma_a1_u2.append([Sigma_a1_val])
                  all_Sigma_a2_u2.append([Sigma_a2_val])
                  all_u2_vals.append([float(u_pt)])

    data_constraint_u1 = PointwiseConstraint.from_numpy(
        nodes=nodes,
        invar={"x": np.array(all_x_u1), "Q": np.array(all_Q_u1), "Sigma_a1": np.array(all_Sigma_a1_u1), "Sigma_a2": np.array(all_Sigma_a2_u1)},
    ...
    data_constraint_u2 = PointwiseConstraint.from_numpy(
        nodes=nodes,
        invar={"x": np.array(all_x_u2), "Q": np.array(all_Q_u2), "Sigma_a1": np.array(all_Sigma_a1_u2), "Sigma_a2": np.array(all_Sigma_a2_u2)},
