...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    de1 = DiffusionEquation1D(u="u1", D=1, Sigma_a=Sigma_a1/D1, Q=Q/D1) #Q/d1 here ?
    ...
    diffusion_net_u1 = instantiate_arch(
        input_keys=[Key("x"), Key("Sigma_a1_hat"), Key("Q_hat")],
        output_keys=[Key("u1")],
    ...
    for Sa1_val in [0.1, 0.2, 0.3]:
        for Q_val in [0.5, 0.8, 1.2]:
                u1 = analytical_solution_1(
                    X1.flatten(), D1, D2, a_ext, Sa1_val, Sigma_a2, Q_val/D1, a1
                )
