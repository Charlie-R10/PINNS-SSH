...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    ### >>> Non-uniform parameterization (denser near RHS)
    #    Using quadratic distribution to emphasize large x
    param = Parameterization(lambda n: np.linspace(0, 1, n) ** 2)
    line = Line1D(min_x, max_x, parameterization=param)

    ode_domain = Domain()

    # Analytical boundary at x=0
    ...
    ### >>> Interior PDE constraint with RHS-weighted loss
    def rhs_weight(invar):
        """Increase loss weight as x → max_x"""
        x_norm = invar["x"] / max_x
        return 1.0 + 4.0 * x_norm  # weight up to ×5 stronger near RHS
    ...
    interior = PointwiseInteriorConstraint(
        ...
        loss_weight_fn=rhs_weight   ### >>> Added weighting
    )
